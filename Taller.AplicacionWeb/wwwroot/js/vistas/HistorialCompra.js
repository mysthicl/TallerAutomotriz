...
$("#btnBuscar").click(function () {
    ...
    fetch(`/Compra/Historial?numeroVenta=${numeroVenta}&fechaInicio=${fechaInicio}&fechaFin=${fechaFin}`)
        .then(response => {
            ...
            if (responseJson.length > 0) {
                responseJson.forEach((venta) => {
                    $("#tbventa tbody").append(
                        $("<tr>").append(
                            $("<td>").text(venta.fecha),
                            $("<td>").text(venta.numeroCompra),
                            $("<td>").text(parseFloat(venta.totalDeLaCompra).toFixed(2)),
                            $("<td>").append(
...
$("#tbventa tbody").on("click", ".btn-info", function () {
    ...
    $("#txtNumVenta").val(s.numeroCompra)
    $("#txtUsuarioRegistro").val(s.usuario)


    $("#txtTotal").val(parseFloat(s.totalDeLaCompra).toFixed(2))

    $("#tbProductos tbody").html("")
    let subtotal = 0;

    s.tblDetalleCompras.forEach((item) => {
    ...
    $("#linkImprimir").attr("href", `/Compra/MostrarPDFCompra?numeroCompra=${s.numeroCompra}`)
